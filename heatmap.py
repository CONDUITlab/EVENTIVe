...
import matplotlib
matplotlib.use('agg') #ensure DISPLAY variable is working
...
def heatmap(data,n_clusters,title="Heatmap",plot_width=500,plot_height=1300): 
    ...
    plot_tools = {'tools':['reset',SaveTool(), hover, "box_select"]}
    #create bokeh figure
    p = figure(plot_width=plot_width, plot_height=plot_height, **plot_tools, title=title,
           y_range=[str(x) for x in list(data.index)], x_range=list(data.columns), #was reversed
           toolbar_location="above", x_axis_location="below")
    p.rect(y="index", x="value", width=1, height=1, source=source,
       line_color=None, alpha=0.8,fill_color=transform('metric', mapper))
    color_bar = ColorBar(color_mapper=mapper, scale_alpha=0.8,location=(0, 0),
    ...
    p.toolbar.logo = None
    # export_png(gridplot([[p,p1]]), filename="plot.png")
...
def getDendrogram(data,n_clusters,plot_height):
    m = data.as_matrix()
    link = linkage(m,"ward") #ward
    if n_clusters == 0: #infer from the data
        den = dendrogram(link)
    else: #use user selected amount
...
def graphDendrogram(data,den,plot_height):
    ...
    #create bokeh figure
    p = figure(plot_width=100, plot_height=plot_height,y_range=(min_val-min_val/2,max_val+min_val/2),
        toolbar_location="above",tools=[SaveTool()])
